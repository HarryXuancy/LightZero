...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        main_config.exp_name = f'data_rezero-policy-collect_ctree/gomoku_b{board_size}_rand{prob_random_action_in_bot}-no_reanalyze'
        from lzero.entry import train_manore
        train_manore([main_config, create_config], seed=seed, max_env_step=max_env_step)
        main_config.exp_name = f'data_rezero-policy-collect_ctree/gomoku_b{board_size}_rand{prob_random_action_in_bot}-low_reanalyze_frequency'
        from lzero.entry import train_marf05
        train_marf05([main_config, create_config], seed=seed, max_env_step=max_env_step)
        main_config.exp_name = f'data_rezero-policy-collect_ctree/gomoku_b{board_size}_rand{prob_random_action_in_bot}-high_reanalyze_frequency'
        from lzero.entry import train_marf2
        train_marf2([main_config, create_config], seed=seed, max_env_step=max_env_step)
